...
const preloadPosters = async (data) => {
	const filenames = data.unique('imdbId')
		.map((d) => [d.imdbId || d.title, `posters/${d.imdbId || d.title}.jpeg`]);
...
const formatString = (str, data) => {
	...
	const output = str
		.replace('{{ TOTAL_TIME }}', formatTime(data.sum('runtime')))
		...
		.replace('{{ NB_MOVIES_VOD }}', unique.filter((d) => isInCategory(d, 'vod')).length)
		...
		.replace('{{ NB_MOVIES_UM }}', unique.filter((d) => isInCategory(d, 'unknownMovies')).length)
...
const getMovieSelection = (movies) => {
	...
	const moviesInTheater = movies.filter((d) => d.medium === 'Cinéma' && d.year === 2021);
...
const isInCategory = (d, category) => {
	switch(category) {
		case 'theater':
			return d.medium === 'Cinéma' && d.year >= 2020;
		case 'festival':
			return d.comment && d.comment.includes('Festival');
		case 'vod':
			return d.medium !== 'Cinéma' && d.year >= 2021;
		case 'top':
			return d.comment === 'Top 10';
		case 'rewatch':
			return d.rewatch;
		case 'discovery':
			return ((d.medium === 'Cinéma' && d.year < 2020) || (d.medium !== 'Cinéma' && d.year <= 2020)) && !d.rewatch;
		case 'unknownMovies':
			return d.comment === 'Unknown Movies';
		default:
			return true;
	}
}
